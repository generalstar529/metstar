...
export default function BetterFeature() {
  ...
  useEffect(() => {
    let imageCards = document.getElementsByClassName(
      "home-feature-imagecards-item"
    );
  ...
  const clickHandler = (index: number, element: Element) => {
    ...
    for (let i = 0; i < imageCards.length; i++) {
      ...
      imageCards[i].children[0].children[2].classList.remove(
        "content-animation"
      );
    }
    ...
    imageCards[index].children[0].children[2].classList.add(
      "content-animation"
    );
  };
  ...
  const scrollHandler = () => {
    ...
    if (
      ...
      if (
        curPos >= imageStartRef.current?.offsetTop &&
        curPos + window.innerHeight <= imageEndRef.current?.offsetTop
      ) {
        if (animationRef.current) {
          animationRef.current.style.marginTop =
            curPos - imageStartRef.current?.offsetTop + "px";
        }
      ...
      <div className="container mx-auto grid grid-cols-1 lg:grid-cols-12 px-11 pb-10">
        <div className="w-full col-span-5 relative">
          ...
          <div className="flex flex-row mt-5">
            ...
            <div
              className="px-2 py-12 mt-0 flex flex-col justify-around"
              ref={(el) => (animationRef.current = el)}
            >
              ...
              <Link href="/capabilities">
                <AnimatedButton
                  variant="transparent"
                  icon="arrow-right"
                  className="text-white text-left font-bold text-sm"
                >
                  SEE ALL
                </AnimatedButton>
              </Link>
