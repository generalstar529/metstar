  ...
  return (
    <section className="capabilities-item-introduction" style={{ zIndex: 1 }}>
      <div className='mx-auto'>
        <TabView activeIndex={currentIndex} className='text-lg' onTabChange={(e) => onChangeTab(e.index)} scrollable>
          {
            capabilitycards.map((card, idx) => (
              <TabPanel key={idx} header={card.topic}>
                <div className='grid gric-cols-1 lg:grid-cols-2 p-5 bg-zinc-100'>
                  ...
                  />
                  <Image src={images[idx]} alt={card.topic} className='layout-image my-auto' />
                </div>
              </TabPanel>
            ))
          }
