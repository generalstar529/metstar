...
export default function Offering() {
  ...
  const titleRefs = Array.from({ length: 8 }).map(() => useRef<HTMLDivElement>(null));
    ...
    <section className="capabilities-offering p-10">
      <div className='container'>
        <div className="flex justify-between">
          <div className="text-3xl font-bold offering-text">CAPABILITY OFFERINGS</div>
          <div className="offering-select">
            <Select options={options} onChange={(e) => onChangeSelect(e.target.selectedIndex)} />
          </div>
        </div>
        {
          offerings.map((offerItem, idx) => (
            <div key={idx} className="my-10" ref={titleRefs[idx]}>
              <div className="text-2xl mb-5">
                <Link href={offerItem.path} className="offering-title font-bold">{offerItem.title}</Link>
              </div>
                ...
                { offerItem.contents.map((contentItems, idx) => (
                  <div key={idx}>
                    { contentItems.map((item, id) => (
                      <div className="py-1" key={id}>{item}</div>
                    )) } 
                  </div>
                )) }
              </div>
