      ...
      <div className="container mx-auto px-11 py-32 grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="col-span-1">
          ...
          <div className="text-black text-sm my-10">
            The energy industry makes modern life possible, but few realize how
            intertwined the electric power, construction and communications
            industries are in our daily lives. Metstar Premier Era Innovations's
            capabilities span the industry to build the infrastructure that
            connects people and power.
          </div>
          {CapabilityMenus.map((item, idx) => (
            <Link key={idx} href={item.path}>
              <div className="font-bold my-4 menu">{item.name}</div>
            </Link>
          ))}
        </div>
        <div className="col-span-1">
          <div className="directory w-9/12 mx-auto p-5">
            ...
            <div className="text-sm my-2">
              View our Capability Directory for a thorough listing of our
              service offerings
            </div>
            <Link href="/capabilities-catalog">
              <AnimatedButton
                variant="warning"
                icon="arrow-right"
                className="flex text-white text-left text-sm my-5"
              >
